...
class GfG 
{
    ...
    public static Tree create(int index, ArrayList<Integer>arr){
        if(index>=arr.size()){
            return null;
        }
        
        Tree root = new Tree(arr.get(index));
        root.left = create(2*index+1,arr);
        root.right = create(2*index+2,arr);
        return root;
    }
